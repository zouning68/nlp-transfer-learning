...
def create_tfrecords(save_dir, basename, data, bsz_per_host, seq_len,
  ...
  while i + seq_len <= data_len:
    ...
    for idx in range(bsz_per_host):
      ...
      feature_infos.append({"input": ' '.join([str(int(e)) for e in cat_data]), "is_masked": ' '.join([str(int(e)) for e in is_masked]), \
                            "target": ' '.join([str(int(e)) for e in tgt]), "seg_id": ' '.join([str(e) for e in seg_id]), "label": [label], \
                            "input_token": ' '.join([sp.id_to_piece(int(e)) for e in cat_data]), "target_token":' '.join([sp.id_to_piece(int(e)) for e in tgt])})
      feature = {
  ...
  record_writer.close();    print(json.dumps(feature_infos[:3], ensure_ascii=False))
  tf.logging.info("Done writing %s. Num of batches: %d", save_path, num_batch)

  with open("data/features", "w", encoding="utf8") as fin: print(json.dumps(feature_infos, ensure_ascii=False), file=fin)

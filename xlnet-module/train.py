...
# Model config
flags.DEFINE_integer("mem_len", default=384,
...
flags.DEFINE_integer("n_layer", default=2,
      help="Number of layers.")
flags.DEFINE_integer("d_model", default=100,
...
flags.DEFINE_integer("d_inner", default=128,
...
def get_model_fn():
  ...
  def model_fn(features, labels, mode, params):
    ...
    if FLAGS.use_tpu:
        train_spec = tf.contrib.tpu.TPUEstimatorSpec(mode=mode, loss=total_loss, train_op=train_op, host_call=host_call, scaffold_fn=scaffold_fn)
    else:
...
def cache_fn():
  ...
  if FLAGS.mem_len > 0:
    for _ in range(FLAGS.n_layer):
      zeros = tf.zeros([FLAGS.mem_len, FLAGS.train_batch_size, FLAGS.d_model], dtype=tf_float)
      mems.append(zeros)
  return mems
